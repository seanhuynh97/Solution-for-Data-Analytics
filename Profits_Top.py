"""
Profits_Top
Purpose: Called from Main. Displays the Top 10 Profitable products in a given time scope
"""
import pandas as pd
...
def top_ten_by_year():
    #Gets 10 most profitable products
    ...
    profit = columns.groupby(['Year', 'Product Name']).sum().sort_values(by='Profit', ascending=False)
    profit = profit.reset_index()

    year = get_year()
    
    select_year = profit.loc[profit['Year'] == year]
    data_title = "\nThe 10 Most Profitable Products of " + str(year) + ":\n "
    chart_title = "10 Most Profitable Products of " + str(year)
    
    print_data(select_year.head(10), data_title, chart_title)

def top_ten_by_year_month():
    #Gets 10 most profitable products by specified month
    excel = pd.ExcelFile("SalesDataFull.xlsx")
    order_data = excel.parse("Orders")
    ...
    profit = columns.groupby(['Year', 'Month', 'Product Name']).sum().sort_values(by='Profit', ascending=False)
    profit = profit.reset_index()
    ...
    data_title = "\nThe 10 Most Profitable Products of " + str(month) + "/" + str(year) + ":\n"
    chart_title = "10 Most Profitable Products of " + str(month) + "/" + str(year) + ": "
    
    print_data(select_year_month.head(10), data_title, chart_title)

def top_ten_by_year_quarter():
    #Gets 10 most profitable products by specified quarter
    excel = pd.ExcelFile("SalesDataFull.xlsx")
    order_data = excel.parse("Orders")
    ...
    profit = columns.groupby(['Year', 'Quarter', 'Product Name']).sum().sort_values(by='Profit', ascending=False)
    profit = profit.reset_index()
    ...
    data_title = "\n10 Most Profitable Products of Quarter " + str(quarter) + " in " + str(year) + ":"
    chart_title = "10 Most Profitable Products of Quarter " + str(quarter) + " in " + str(year) + ":"
    
    print_data(select_year_quarter.head(10), data_title, chart_title)

def top_ten_all():
    #Gets 10 most profitable products for all years
    ...
    col_profits = col_prof.groupby(["Product Name"]).sum().sort_values(by=["Profit"], ascending=False)
    col_profits = col_profits.reset_index()

    data_title = "\n10 Most Profitable Products of All Time:\n"
    chart_title = "10 Most Profitable Products of All Time"
    
    print_data(col_profits.head(10), data_title, chart_title)

def top_ten():
    #Menu display for Most Profitable Products
    profit_loop = True
